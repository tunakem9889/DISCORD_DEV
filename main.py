...
class MusicPlayer:
    ...
    def get_queue_info(self):
        if not self.queue and not self.current_song:
            return "Không có bài hát nào trong queue"
...
async def get_youtube_info(url):
    ...
    try:
        ...
        print(f"Lỗi khi lấy thông tin YouTube: {e}")
        return None
...
async def add_song_to_queue(ctx, url, voice_client, guild_id):
    ...
    if not re.match(r'https?://(?:www\.)?(?:youtube\.com|youtu\.be)', url):
        await ctx.send("❌ Chỉ hỗ trợ URL YouTube!")
        return
        
    await ctx.send("🔍 Đang tìm kiếm bài hát...")
    
    # Lấy thông tin bài hát
    song_info = await get_youtube_info(url)
    if not song_info:
        await ctx.send("❌ Không thể lấy thông tin bài hát!")
        return
    ...
    embed.add_field(name="📺 Link", value=f"[YouTube]({song_info['webpage_url']})", inline=False)
...
async def play(ctx, *, url):
    ...
    if not url:
        await ctx.send("❌ Hãy cung cấp URL YouTube!")
        return
...
async def now(ctx):
    ...
    embed.add_field(name="📺 Link", value=f"[YouTube]({song['webpage_url']})", inline=False)
...
async def remove(ctx, index: int):
    ...
    if index < 1 or index > len(player.queue):
        await ctx.send(f"❌ Vị trí không hợp lệ! Queue có {len(player.queue)} bài hát.")
        return
        
    # Xóa bài hát khỏi queue (index bắt đầu từ 1)
    removed_song = player.queue[index - 1]
    del player.queue[index - 1]
    
    await ctx.send(f"🗑️ Đã xóa bài hát: **{removed_song['title']}**")
...
async def shuffle(ctx):
    ...
    if len(player.queue) < 2:
        await ctx.send("❌ Cần ít nhất 2 bài hát để xáo trộn!")
        return
